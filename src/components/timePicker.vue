      ...
      <template v-if="selectableRange" class="select-container">
        <div class="select">
          <ul ref="hour" class="line line1">
            <li class="line1-item" v-for="(item, index) in line1List" :key="index" :ref="'line1-'+item" >{{zero(item)}}</li>
          </ul>
          <ul ref="minute" class="line line2">
            <li class="line1-item" v-for="(item, index) in line2List" :key="index" :ref="'line2-'+item">{{zero(item)}}</li>
          </ul>
          <ul ref="second" class="line line3">
            <li class="line1-item" v-for="(item, index) in line3List" :key="index" :ref="'line3-'+item">{{zero(item)}}</li>
          </ul>
          <div class="middle-line"></div>
        </div>
        <div class="control">
          <span @click="handleCancle">取消</span>
...
export default {
  ...
  watch: {
    popoverShow (newVal) {
      ...
      if (this.selectableRange && newVal) {
        this.list1OffsetTop = []
        this.list2OffsetTop = []
        this.list3OffsetTop = []
        this.$nextTick(() => {
  ...
  methods: {
              ...
              llist.forEach((bar, index) => {
                if (top > bar) {
                  if (item === 'hour') {
                    ...
                    this.selectTime = this.zero(index) + ':' + this.zero(this.minute) + ':' + this.zero(this.second)
                  } else if (item === 'minute') {
                    this.minute = index
                    this.$emit('input', this.zero(this.hour) + ':' + this.zero(index) + ':' + this.zero(this.second))
                    this.selectTime = this.zero(this.hour) + ':' + this.zero(index) + ':' + this.zero(this.second)
                  } else {
                    this.second = index
                    this.$emit('input', this.zero(this.hour) + ':' + this.zero(this.minute) + ':' + this.zero(index))
                    this.selectTime = this.zero(this.hour) + ':' + this.zero(this.minute) + ':' + this.zero(index)
                  }
                }
              })
              timeout = setTimeout(() => {
    ...
    handleCancle () {
      ...
      this.selectTime = this.valueClone
      this.popoverShow = false
    },
    handleConfirm () {
      this.$emit('input', this.zero(this.hour) + ':' + this.zero(this.minute) + ':' + this.zero(this.second))
      this.selectTime = this.zero(this.hour) + ':' + this.zero(this.minute) + ':' + this.zero(this.second)
      this.popoverShow = false
    },
    handleClick (item) {
